...
- ✅ Build Event Simulator for chat-export.json processing
...
export class EventSimulator extends EventEmitter {
  private data: ChatExportData | null = null;
  ...
  async loadChatExportData(filePath: string): Promise<void> {
    try {
  ...
  startSimulation(speed: number = 100): void {
    if (!this.data) {
      throw new Error('No data loaded. Call loadChatExportData() first.');
    }
  ...
  private simulateNextMessage(): void {
    ...
    const chatExportMsg = this.data.messages[this.currentIndex];
    
    // Skip non-message types
    if (chatExportMsg.type !== 'message' || !chatExportMsg.text) {
      this.currentIndex++;
      setImmediate(() => this.simulateNextMessage());
      return;
    }

    // Convert community message to simulated event
    const simulatedEvent = this.convertToSimulatedEvent(chatExportMsg);
  ...
  private convertToSimulatedEvent(chatExportMsg: ChatMessage): SimulatedEvent {
    // Extract text content
    let content = '';
    if (typeof chatExportMsg.text === 'string') {
      content = chatExportMsg.text;
    } else if (Array.isArray(chatExportMsg.text)) {
      content = chatExportMsg.text
    ...
    const userId = parseInt(chatExportMsg.from_id.replace('user', ''));

    return {
      id: chatExportMsg.id,
      content: content.trim(),
      userId: userId,
      timestamp: new Date(chatExportMsg.date),
      replyToMessageId: chatExportMsg.reply_to_message_id,
      groupId: this.data!.id
    };
  }
...
async function testEventSimulator() {
  ...
  try {
    // Load community data
    await simulator.loadChatExportData('./chat-export.json');
...
async function runPhase1System() {
  ...
  try {
    ...
    console.log('\n📁 Step 3: Loading the community Data');
    await eventSimulator.loadChatExportData('./chat-export.json');
...
✅ **Event Simulation**: Successfully processes chat-export.json messages  
