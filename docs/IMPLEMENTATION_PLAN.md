...
## Overview
The system will process Telegram-like messages from `chat-export.json`, accumulate events in Redis, and perform intelligent topic categorization using a multi-agent system with versioned in-memory graph storage.
...
```
the community.json → Event Simulator → Accumulator Raft → Batch Dump → Concierge Agent (Running Process with In-Memory Tree) → Individual Agents → Storage
```
...
- **Purpose**: Parse chat-export.json and emit events simulating real-time message flow
...
2. **Event Simulation**
   - Parse chat-export.json structure
...
├── simulators/
│   └── eventSimulator.ts           (the community.json simulation)
├── postgresClient/
...
1. **Event Simulator**: Begin parsing chat-export.json and emitting events
...
4. **Event Simulator**: Begin streaming chat-export.json data

### 2. Message Processing Flow
1. **Event Simulator** → Parses chat-export.json → Emits message events
