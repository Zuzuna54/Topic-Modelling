...
```
the community.json → Event Simulator → Accumulator Raft → Concierge Agent (Meta-Agent) → Stateless Processing Agents
                                        ↓                         ↓
...
└── chat-export.json                  # 📋 Sample chat data for simulation
```
...
# Run event simulation with chat-export.json
