...
export class ConciergeAgent extends EventEmitter {
  ...
  private async processMessageBatch(batchEvent: BatchReadyEvent): Promise<void> {
    ...
    const [embeddings, sentimentResults, contextualToxicityResults, relationshipUpdates] = await Promise.all([
      this.embeddingAgent.generate(processedTexts),
      this.sentimentAgent.analyzeWithContext(processedTexts, messageContexts),
      this.toxicityAgent.analyzeWithContext(processedTexts, messageContexts),
    ...
    await this.updateGraph(cleanMessages, {
      embeddings,
      sentimentResults,
      toxicityResults: contextualToxicityResults,
      topicResults,
      relationshipUpdates
    });
  ...
  private async updateGraph(
    ...
    results: {
      ...
      toxicityResults: ContextualToxicityResult[];
    ...
    messages.forEach((simMessage, index) => {
      const message: Message = {
        ...
        toxicity: toxicityResults[index]?.contextualToxicity,
      ...
      if (!user) {
        ...
        user = {
          id: message.userId,
          name: `User${message.userId}`, // We don't have real names from chat-export.json
          messageIds: new Set(),
